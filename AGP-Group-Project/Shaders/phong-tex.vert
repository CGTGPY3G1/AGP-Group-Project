...
void main(void) {
	...
	ex_N = normalize(normalmatrix * normal);
	mat3 TBN = transpose(mat3(normalize(normalmatrix * tangent), normalize(normalmatrix * bitangent), ex_N));
	// L - to light source from vertex
	ex_L = normalize(TBN * (lightPosition.xyz - vertexPosition.xyz));
